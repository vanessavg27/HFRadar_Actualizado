...
def Ruido_oficial(ch0,nrange):
    
    med_profile = np.median(ch0.T,axis=0)
    med_total   = np.median(ch0.T)

    index_med   = [index for index,value in enumerate(med_profile) if value < 0.99*med_total]
...
location = {'11':{'Campaign':{'ch0':[[86,86,86],[38,38,37]],'ch1':[[86,86,86],[38,38,37]]},
                    'Normal':{'ch0':[[92,92,92],[15,16,15]],'ch1':[[93,93,93],[16,16,16]]}}, #JROA Good
            '12':{'Campaign':{'ch0':[[86,86,86],[38,38,37]],'ch1':[[86,86,86],[38,38,37]]},
                    'Normal':{'ch0':[[92,92,92],[15,16,15]],'ch1':[[93,93,93],[16,16,16]]}}, #JROB Good
            '21':{'Campaign':{'ch0':[[86,86,86],[38,38,38]],'ch1':[[86,86,86],[38,38,38]]},
                    'Normal':{'ch0':[[91,91,91],[30,30,30]],'ch1':[[95,95,95],[18,18,18]]}}, #HYOA Good
            '22':{'Campaign':{'ch0':[86,38],'ch1':[86,38]},'Normal':{'ch0':[92,28],'ch1':[95,18]}}, #HYOB
            '31':{'Campaign':{'ch0':[88,38],'ch1':[86,38]},'Normal':{'ch0':[92,20],'ch1':[94,20]}}, #Mala
            '41':{'Campaign':{'ch0':[88,38],'ch1':[86,38]},
                  
                  'Normal':{'ch0':[92,20],'ch1':[94,20]}}, #Merced
            ...
            '61':{'Campaign':{'2.72':{'ch0':[[87,86,87],[40,38,37]],'ch1':[[86,85,86],[38,38,38]]},
                              '3.64':{'ch0':[[90,87,88],[37,36,35]],'ch1':[[86,86,86],[38,38,38]]}},
                    'Normal':{'2.72':{'ch0':[[93,93,86],[19,19,12]],'ch1':[[95,95,92],[18,18,18]]}, 
                              '3.64':{'ch0':[[93,93,86],[19,19,12]],'ch1':[[95,95,92],[18,18,18]]}}}  #Oroya  Good                                     
            }
            #location[lo]['Normal']["%s"%(freqs)]['ch0'][0][code]
...
for CurrentSpec in files:
    ...
    if hild_sk == 1:
        ...
        Noise_a  = Ruido_oficial(ch0,nrange)
        Noise_b  = Ruido_oficial(ch1,nrange)
    ...
    if nc == 100:
        ...
        percentil_a  = location[lo]['Normal']['ch0'][0][code]
        percentil_b  = location[lo]['Normal']['ch1'][0][code]
        #Vecinos del algorimo DBSCAN
        vecinos_a    = location[lo]['Normal']['ch0'][1][code]
        vecinos_b    = location[lo]['Normal']['ch1'][1][code]
    else:
        #Campaign Mode
        #NoiseFloor_a = np.median( FullSpectra_a, axis=0)
        #NoiseFloor_b = np.median( FullSpectra_b, axis=0)
        percentil_a  = location[lo]['Campaign']["%s"%(freqs)]['ch0'][0][code]
        percentil_b  = location[lo]['Campaign']["%s"%(freqs)]['ch1'][0][code]
        #Vecinos del algorimo DBSCAN
        vecinos_a    = location[lo]['Campaign']["%s"%(freqs)]['ch0'][1][code]
        vecinos_b    = location[lo]['Campaign']["%s"%(freqs)]['ch1'][1][code]
