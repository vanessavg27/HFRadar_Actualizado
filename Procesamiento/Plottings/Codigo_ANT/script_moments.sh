...
#date_to_process="2022/06/22" # Modificar y quitar el comentario si se desea procesar otra fecha
###################Generating Moments Data#################################################3
echo "Starting Moments"
screen -S "MOMENTS_HF" -d -m ./GenerateMoments.sh "/media/igp-114/PROCDATA/" $1 $2 $date_to_process
sleep 1
...
##################Plotting Parameters RTDI, SNR, DOPLLER y OUT #################################################3
echo "Plotting Parameters from Reducted Data"
screen -S "PlottingParam_Floyd" -d -m ./Plot_RTDI_OUT.sh  "/media/igp-114/PROCDATA/MomentsFloyd/" $1 $2 $date_to_process
sleep 1
