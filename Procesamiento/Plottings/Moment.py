...
parser.add_argument('-lo',action='store',dest='lo_seleccionado',type=int,help='Parametro para establecer la ubicacion de la estacion de Rx y su orientacion.\
										Example: XA   ----- X: Es el primer valor determina la ubicacion de la estacion. A: Es el segundo valor determina la orientacion N45O o N45E.  \
										11: JRO-N450, 12: JRO-N45E \
									    21: HYO-N45O, 22: HYO-N45E', default=12)
########################## GRAPHICS - RESULTS  ###################################################################################################
parser.add_argument('-path_o',action='store',dest='path_out',help='Directorio de Resultados \
					.Por defecto, se esta ingresando entre comillas /media/soporte/PROCDATA/MomentsFloyd/', default="/media/soporte/PROCDATA/MomentsFloydFilt/")
...
graphics_folder = results.path_out
R          = results.reduccion
    ...
    for i in range(len(index_peak)-1):
        for j in range(len(index_peak)-1):
            if snr[index_peak[j+1]]>snr[index_peak[j]]:
                ...
                index_peak[j+1] = val
    #print("B:",b)
    index_peak = [ n for n in index_peak if (n > high_index and n < 701)]
    return index_peak
