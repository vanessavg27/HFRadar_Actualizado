...
def incompletedays(data, rxcode,out_or_figure):
    #data_in must be  a list tlike this:
...
def File_writer(dias_faltantes,station_type,datatype,year,rxcode):
    ...
    #os.remove("Doys_not_sent_%s_%s"%(datatype,station_type))
    long = len(dias_faltantes[Tx_s[0]])
...
def dias_faltantes(ruta_base, rxcode,rxname,datatype):
    #station_rx = rxname
    ...
    for tx in tx_s:
        ...
        for month in range(1,date_month+1):
            ...
            sorted(datos_dias)
            days_off += notdays(datos_dias)
            Days_off_tx[tx]=days_off

            days_incomplete += incompletedays(datos_dias, rxcode,out_or_figure)
...
#dias_faltantes(ruta_base, rxcode,rxname,datatype)
...
for file in dlist[:]: # Se usa una copia porque dlist puede ser modificado dentro del bucle
	...
	arch_files= glob.glob("%s/%s/%s*.%s"%(graph_freq0, doy,str_datatype,extension))
	...
	if datatype == "params":
		...
		file_1 = arch_files[0][:-1].split("/")[-3]
	else:
		#file_1 = jpg_files[0][:-1].split("/")[-2]
		file_1 = arch_files[0][:-1].split("/")[-2]
	file_1 = file_1[1:]
	print("FILE: ",file_1)
	...
	#print("%s/%s/%s%s%s%s%s*.%s wmaster@jro-app.igp.gob.pe:%s"%(graph_freq0,doy,str_datatype,
	#letter,YEAR, DAYOFYEAR_str, rxcode, extension, remote_folder))
	

	temp_command0 = "scp -r -P 6633 %s/%s/%s%s%s%s%s*.%s wmaster@jro-app.igp.gob.pe:%s"%(graph_freq0,doy,str_datatype,
	letter,YEAR, DAYOFYEAR_str, rxcode, extension, remote_folder)
	#print("Comando 01: ", temp_command0)
	#print(sendBySCP(temp_command0, rxcode))
	...
	temp_command1 = "scp -r -P 6633 %s/%s/%s%s%s%s%s*.%s wmaster@jro-app.igp.gob.pe:%s"%(graph_freq1,doy,str_datatype,
	letter,YEAR, DAYOFYEAR_str, rxcode, extension, remote_folder)
	#print(temp_command1)
