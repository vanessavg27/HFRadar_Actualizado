...
class Reduccion():
    ...
    def clean_run(self):
        ...
        for CurrentSpec in self.h5_list(self.code,self.ngraph):
            ...
            #### Uso del metodo estadìstico percentil segun cada estacion Rx
...
#           ##############################################################
            ...
            if self.reducted == 1:
                ...
                print("       Guardado hdf5: ",' %s'%(CurrentSpec.split("/")[-1][:-5]))
    ...
    def multifreqs(self,lo,mode,freqs,ch,variable,code):
        ...
        location = {'11':{'Campaign':{'2.72':{'ch0':[[86,86,86],[38,38,37]],'ch1':[[86,86,86],[38,38,37]]},
                    ...
                    '12':{'Campaign':{'2.72':{'ch0':[[88,94,90],[38,38,36]],'ch1':[[86,92,90],[38,38,37]]},
                            ...
                            'Normal':{'2.72':{'ch0':[[93,92,92],[23,21,18]],'ch1':[[93,93,93],[22,22,18]]},
...
#Funciones de relleno con padding
    ...
    def padding(self,df,pad,nc):
        ...
        index_min = int(round((-3.32-Freqs[0])/(Freqs[1]-Freqs[0])))
        index_max = int(round(( 3.32-Freqs[0])/(Freqs[1]-Freqs[0])))
        self.df_out          = pd.DataFrame()
        #DATA "DBSCAN")
        ancho_min = int(round((-3-Freqs[0])/(Freqs[1]-Freqs[0])))
        ancho_max = int(round(( 3-Freqs[0])/(Freqs[1]-Freqs[0])))
        #print("DataFrame:",df)
        df = df.sort_values(by=['Rango','Frecuencia'],ascending=[True,True])
        range_freq = []
        data_export= []
        for h in range(1000):
            a = df[df['Rango']==h]
            if a.empty:
                continue
            if h > 50:       #Considerar todas las frecuencias en la altura 50*1.5 y 400*1.5 km
            ...
            for l in lista_0:
                range_freq.append(self.relleno(l,pad,index_min,index_max,h))
    
        for range_f in range_freq:  
            data_export.append([[range_f[0],range_f[1][i],3] for i in range(len(range_f[1]))])

        data_export = [i for altura in data_export for i in altura]
        self.df_out = pd.DataFrame(data_export,columns=["Rango","Frecuencia","label"])
